...
(
    ...
    # override the cwd and inspect the active project via ambient session state.
    "$GATE_SCRIPT" --project-root "$T3_DIR/project"
) > "$T3_DIR/out.txt" 2>&1
EXIT3=$?
set -e
...
if grep -q "CLAUDE_PROJECT_DIR" "$TEST_DIR/help.txt" 2>/dev/null && grep -q "git worktree root" "$TEST_DIR/help.txt" 2>/dev/null; then
    pass "rlcr-stop-gate --help documents the git worktree root default"
else
    OUTPUT5=$(cat "$TEST_DIR/help.txt" 2>/dev/null || true)
    fail "rlcr-stop-gate --help documents the git worktree root default" "help output mentioning CLAUDE_PROJECT_DIR and git worktree root" "$OUTPUT5"
fi
